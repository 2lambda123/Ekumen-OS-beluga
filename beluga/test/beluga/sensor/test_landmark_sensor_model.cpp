...
LandmarkModelParam getDefaultModelParams() {
...
TYPED_TEST(LandmarkSensorModelTests, BullsEyeDetection) {
  ...
  TypeParam uut(getDefaultModelParams(), LandmarkMap(default_map_boundaries, {{3.0, -2.0, 0.0, 0}}));
...
TYPED_TEST(LandmarkSensorModelTests, MultipleBullsEyeDetections) {
  ...
  EXPECT_NEAR(
      expectedAggregatedProbability({1.0, 1.0, 1.0, 1.0}),
      uut.importance_weight(getRobotInWorldTransform<typename TypeParam::state_type>()), 1e-02);
}

TYPED_TEST(LandmarkSensorModelTests, OneStdInRange) {
  // test case where the landmark is 1 std offset from the expected range
  TypeParam uut(getDefaultModelParams(), LandmarkMap(default_map_boundaries, {{1.0, -11.0, 0.0, 0}}));
  // baseline
...
TYPED_TEST(LandmarkSensorModelTests, NoSuchLandmark) {
  ...
  TypeParam uut(getDefaultModelParams(), LandmarkMap(default_map_boundaries, {{0.0, 1.0, 0.0, 99}}));

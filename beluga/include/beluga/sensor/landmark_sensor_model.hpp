...
struct LandmarkModelParam {
  double sigma_range{0.2};    ///< Standard deviation of the range error.
...
class LandmarkSensorModel : public Mixin {
 public:
  ...
  [[nodiscard]] weight_type importance_weight(const state_type& state) const {
    ...
    const auto detection_weight = [this, &robot_in_world_transform](const auto& point) {
      // calculate range and detection_in_robot_bearing to the detection from the robot
      // the detection is already in robot frame
      const auto [px, py, pz, id] = point;
      const auto detection_range = std::sqrt(px * px + py * py + pz * pz);
      const auto detection_in_robot_bearing = Eigen::Vector3d{px, py, pz}.normalized();

      // convert the detection to the world frame to query the map
      const auto detection_in_world_position = robot_in_world_transform * Eigen::Vector3d{px, py, pz};

      // find the closest matching landmark in the world map
      const auto landmark_opt = landmark_map_.find_nearest_landmark(std::make_tuple(
          detection_in_world_position.x(),  //
          detection_in_world_position.y(),  //
          detection_in_world_position.z(),  //
          id));
      ...
      const auto landmark_in_world_position = Eigen::Vector3d{w_lx, w_ly, w_lz};
      const auto landmark_in_robot_position = robot_in_world_transform.inverse() * landmark_in_world_position;

      const auto landmark_range = landmark_in_robot_position.norm();
      const auto landmark_bearing_in_r = landmark_in_robot_position.normalized();

      // calculate the aperture angle between the detection and the landmark
      const auto cos_aperture = landmark_bearing_in_r.dot(detection_in_robot_bearing);
      const auto sin_aperture = landmark_bearing_in_r.cross(detection_in_robot_bearing).norm();
      const auto bearing_error = std::atan2(sin_aperture, cos_aperture);

      const auto range_error = detection_range - landmark_range;

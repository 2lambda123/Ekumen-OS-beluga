...
#include "beluga_example/tf2_sophus.hpp"

namespace beluga_example {

using LandmarkBasedMonteCarloLocalizationInterface2d = beluga::mixin::compose_interfaces<
    beluga::BaseParticleFilterInterface,
    beluga::StorageInterface<Sophus::SE2d, beluga::Weight>,
    beluga::EstimationInterface2d,
    beluga::OdometryMotionModelInterface2d,
    beluga::BearingSensorModelInterface<beluga::LandmarkMap>>;

using LandmarkBasedMonteCarloLocalizationFilter = ciabatta::mixin<
    beluga::BootstrapParticleFilter,
    ciabatta::curry<beluga::StructureOfArrays, Sophus::SE2d, beluga::Weight, beluga::Cluster>::mixin,
    ...
    ciabatta::curry<beluga::BearingSensorModel2d, beluga::LandmarkMap>::mixin,
    ciabatta::provides<LandmarkBasedMonteCarloLocalizationInterface2d>::template mixin>;

void ExtractParticleCloud(
    const LandmarkBasedMonteCarloLocalizationInterface2d& particle_filter,
  ...
  for (const auto& [state, weight] :
       ranges::views::zip(particle_filter.states_view(), particle_filter.weights_view())) {
...
class LandmarkBearingBasedMonteCarloLocalizationNode : public rclcpp::Node {
 public:
  explicit LandmarkBearingBasedMonteCarloLocalizationNode(const rclcpp::NodeOptions& options)
...

    {
      ...
      auto callback = std::bind(&LandmarkBearingBasedMonteCarloLocalizationNode::timer_callback, this);
      timer_ = create_wall_timer(200ms, std::move(callback), common_callback_group);
    }

    {
      using namespace std::placeholders;
      const auto qos = rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable();
      auto callback = std::bind(&LandmarkBearingBasedMonteCarloLocalizationNode::map_callback, this, _1);
      feature_map_sub_ = create_subscription<beluga_feature_map_server_msgs::msg::DiscreteFeatureMap>(
          "feature_map", qos, std::move(callback), common_subscription_options);
      RCLCPP_INFO(get_logger(), "Subscribed to %s topic", feature_map_sub_->get_topic_name());
    }

    {
      using namespace std::placeholders;
      auto callback = std::bind(&LandmarkBearingBasedMonteCarloLocalizationNode::initial_pose_callback, this, _1);
      initial_pose_sub_ = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
          "initialpose", rclcpp::SystemDefaultsQoS(), std::move(callback), common_subscription_options);

      RCLCPP_INFO(get_logger(), "Subscribed to %s topic", initial_pose_sub_->get_topic_name());
    }

    {
      ...
      feature_detections_connection_ = feature_detections_filter_->registerCallback(
          std::bind(&LandmarkBearingBasedMonteCarloLocalizationNode::feature_detections_callback, this, _1));
 ...
 private:
  void feature_detections_callback(
    ...
    if (!particle_filter_) {
      RCLCPP_WARN_THROTTLE(
          ...
          "Ignoring detected features because the particle filter has not been initialized");
      return;
    }
    ...
    try {
      ...
      tf2::convert(
          tf_buffer_->lookupTransform(kOdomFrameID, kBaseFrameID, timestamp).transform, odom_to_base_transform);
    ...
    try {
      ...
      tf2::convert(
          tf_buffer_->lookupTransform(kBaseFrameID, sensor_frame_id, timestamp).transform, base_to_sensor_transform);
    } catch (const tf2::TransformException& error) {
      RCLCPP_ERROR(get_logger(), "Could not transform from base to features: %s", error.what());
      return;
    }
    ...
    if (force_update_ || (moved_enough && have_enough_features)) {
      ...
      LandmarkBasedMonteCarloLocalizationFilter::measurement_type measurement;
      measurement.reserve(features->positions.size());
      for (size_t i = 0; i < features->positions.size(); ++i) {
        measurement.emplace_back(
            features->positions[i].x, features->positions[i].y, features->positions[i].z, features->categories[i]);
      }
    ...
    if (did_update) {
      ...
      RCLCPP_INFO(
          get_logger(), "Particle filter update iteration stats: %ld particles %ld features - %.3fms",
    ...
    if (last_map_to_odom_transform_) {
      ...
      const auto expiration_stamp =
          tf2_ros::fromMsg(features->header.stamp) + tf2::durationFromSec(kTransformTolerance);
  ...
  void map_callback(const beluga_feature_map_server_msgs::msg::DiscreteFeatureMap::ConstSharedPtr& feature_map) {
    ...
    limiter_params.spatial_hasher =
        beluga::spatial_hash<Sophus::SE2d>{0.5, 0.5, 10. * Sophus::Constants<double>::pi() / 180.};
    ...
    sensor_model_params.sigma_bearing = 0.4;
    sensor_model_params.sensor_in_robot_transform = Sophus::SE3d{
        Sophus::SO3d::rotY(40 * Sophus::Constants<double>::pi() / 180.),  //
        Eigen::Vector3d{0.14, 0.0, 0.34}                                  //
    };
    ...
    beluga::LandmarkMap::landmark_vector landmarks;
    landmarks.reserve(feature_map->positions.size());
    for (size_t i = 0; i < feature_map->positions.size(); ++i) {
      xmin = std::min(feature_map->positions[i].x, xmin);
      ymin = std::min(feature_map->positions[i].y, ymin);
    ...
    auto extents = beluga::LandmarkMap::map_boundaries(xmin, xmax, ymin, ymax, 0.0, 1.0);

    particle_filter_ = std::make_unique<LandmarkBasedMonteCarloLocalizationFilter>(
        sampler_params, limiter_params, motion_model_params, sensor_model_params,
        beluga::LandmarkMap{extents, landmarks});
  ...
  void initial_pose_callback(const geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr& message) {
    if (!particle_filter_) {
  ...
  void initialize(const Sophus::SE2d& pose, const Eigen::Matrix3d& covariance) {
    try {
      ...
      particle_filter_->initialize_states(ranges::views::generate([&distribution]() mutable {
  ...
  std::unique_ptr<LandmarkBasedMonteCarloLocalizationInterface2d> particle_filter_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(beluga_example::LandmarkBearingBasedMonteCarloLocalizationNode)

...
using LandmarkBasedMonteCarloLocalizationInterface2d = beluga::mixin::compose_interfaces<
    ...
    beluga::LandmarkBearingSensorModelInterface<beluga::LandmarkMap>>;
...
using LandmarkBasedMonteCarloLocalizationFilter = ciabatta::mixin<
    ...
    ciabatta::curry<beluga::LandmarkBearingSensorModel, beluga::LandmarkMap>::mixin,
...
class LandmarkBasedMonteCarloLocalizationNode : public rclcpp::Node {
 public:
  explicit LandmarkBasedMonteCarloLocalizationNode(const rclcpp::NodeOptions& options)
...

    {
      ...
      auto callback = std::bind(&LandmarkBasedMonteCarloLocalizationNode::timer_callback, this);
...

    {
      ...
      auto callback = std::bind(&LandmarkBasedMonteCarloLocalizationNode::map_callback, this, _1);
    ...
    {
      using namespace std::placeholders;
      auto callback = std::bind(&LandmarkBasedMonteCarloLocalizationNode::initial_pose_callback, this, _1);
 ...
 private:
  void feature_detections_callback(
    ...
    if (force_update_ || (moved_enough && have_enough_features)) {
      ...
      for (size_t i = 0; i < features->positions.size(); ++i) {
        ...
        auto feature_position_in_base_frame = base_to_sensor_transform * feature_position_in_sensor_frame;
        measurement.emplace_back(
            feature_position_in_base_frame.x(), feature_position_in_base_frame.y(), feature_position_in_base_frame.z(),
            features->categories[i]);
      }
  ...
  void map_callback(const beluga_feature_map_server_msgs::msg::DiscreteFeatureMap::ConstSharedPtr& feature_map) {
    ...
    auto sensor_model_params = beluga::LandmarkBearingModelParam{};
    sensor_model_params.sigma_bearing = 0.20;
    sensor_model_params.sensor_in_robot = Sophus::SE3d{
        Sophus::SO3d::rotY(40 * Sophus::Constants<double>::pi() / 180.),  //
        Eigen::Vector3d{0.14, 0.0, 0.29}                                  //
    };
...
RCLCPP_COMPONENTS_REGISTER_NODE(beluga_example::LandmarkBasedMonteCarloLocalizationNode)

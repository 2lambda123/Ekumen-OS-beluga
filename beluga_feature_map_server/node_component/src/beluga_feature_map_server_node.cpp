...
BelugaFeatureMapServerNode::BelugaFeatureMapServerNode(const rclcpp::NodeOptions& options)
  ...
  RCLCPP_INFO(get_logger(), "Feature map server node starting");
  {
    ...
    declare_parameter(kMapFileParameter, rclcpp::ParameterValue(""), descriptor);
  }

  map_publisher_ = this->create_publisher<beluga_feature_map_server_msgs::msg::DiscreteFeatureMap>(
      "feature_map", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());

  marker_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
      "feature_markers", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());

  load_map_service_ = create_service<std_srvs::srv::Trigger>(
      kMapReloadService, std::bind(
                             &BelugaFeatureMapServerNode::reloadMapServiceCallback, this, std::placeholders::_1,
                             std::placeholders::_2, std::placeholders::_3));

  map_loader_ = std::make_unique<YamlFeatureMapLoader>(get_parameter(kMapFileParameter).as_string());

  RCLCPP_INFO_STREAM(get_logger(), "Feature map source: " << get_parameter(kMapFileParameter).as_string());
  ...
  if (reloadAndPublish()) {
    ...
    RCLCPP_ERROR(
        get_logger(),
        "The feature map could not be loaded from the "
...
void BelugaFeatureMapServerNode::publishMarkers(
    const beluga_feature_map_server_msgs::msg::DiscreteFeatureMap& feature_map) {
  constexpr double kSideLength = 0.1;
  int seq_id = 0;
  static int max_seq_id = 0;

  visualization_msgs::msg::MarkerArray msg;
  for (const auto& feature_pose : feature_map.positions) {
    visualization_msgs::msg::Marker marker;
    marker.header.frame_id = feature_map.header.frame_id;
    marker.header.stamp = feature_map.header.stamp;
    marker.id = seq_id++;
...
RCLCPP_COMPONENTS_REGISTER_NODE(beluga_feature_map_server::BelugaFeatureMapServerNode)

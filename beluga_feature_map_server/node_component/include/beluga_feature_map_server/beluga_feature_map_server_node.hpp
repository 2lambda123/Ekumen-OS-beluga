...
class BelugaFeatureMapServerNode : public rclcpp::Node {
 public:
   ...
   */
  explicit BelugaFeatureMapServerNode(const rclcpp::NodeOptions& options = rclcpp::NodeOptions());

 private:
  std::unique_ptr<FeatureMapLoaderInterface> map_loader_;

  rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr load_map_service_;
  rclcpp::Publisher<beluga_feature_map_server_msgs::msg::DiscreteFeatureMap>::SharedPtr map_publisher_;
  ...
  void publishMarkers(const beluga_feature_map_server_msgs::msg::DiscreteFeatureMap& map);
};

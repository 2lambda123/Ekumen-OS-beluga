...
YamlFeatureMapLoader::YamlFeatureMapLoader(const std::string& map_file) : map_file_{map_file} {}

std::optional<beluga_feature_map_server_msgs::msg::DiscreteFeatureMap> YamlFeatureMapLoader::loadMap() {
  ...
  for (const auto& feature : root_data.features) {
    ...
    if (has_position != must_have_position || has_orientation != must_have_orientation ||
    ...
    if (has_orientation) {
      map.orientations.push_back(convert_orientation(feature.orientation.value()));
    }
    if (has_category) {

...
struct YamlFeatureMapLoaderTests : public testing::Test {
  ...
  void setupTestFile(const std::string& filename, const std::string& file_contents) {
...
TEST_F(YamlFeatureMapLoaderTests, UnbalancedFields) {
  const auto filename = createRandomFilename();
  setupTestFile(filename, serialized_input_unbalanced_fields_in_different_features);
